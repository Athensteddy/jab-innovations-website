...
async function loadAdminOrders(){
  try{
    ...
    rows.innerHTML = adminOrders.map(o => {
      const items = (o.items || [])
        .map(i => `${esc(i.product_name)} × ${Number(i.quantity || 0)}`)
        .join('<br>');
      ...
      if(o.status !== 'confirmed' && o.status !== 'shipped'){
        actions +=
          `<button class="btn" onclick="confirmOrder('${o.id}')">
            Confirm
          </button>`;
      }
      ...
      if(o.status !== 'shipped'){
        actions +=
          ` <button class="btn secondary" onclick="shipOrder('${o.id}')">
            Ship + Tracking
          </button>`;
      }
      ...
      return `
        <tr>
          ...
          <td>
            ...
            <span class="muted">${esc(o.customer_phone || '')}</span>
          </td>
